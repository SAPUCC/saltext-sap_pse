...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/sap_pse/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/sap_pse/*")
            ...
            #    "--include=src/saltext/sap_pse/*,tests/*"
            # )
        finally:
...
def gen_api_docs(session):
    ...
    session.run(
        ...
        "docs/ref/",
        "src/saltext",
        "src/saltext/sap_pse/config/schemas",
    )
